...
// - El primer día cuenta solo si contarPrimero; el último solo si contarUltimo.
//   (Si llevan y devuelven el mismo día, ese día siempre se cobra.)
...
export async function calcularVarios(alqs, hasta) {
  ...
  for (const alq of alqs) {
    ...
    const detalleItems = items.filter(it => it.alquiler_id === alq.id).map(it => {
      // Las piezas agregadas después del inicio cobran desde su propia fecha
      ...
      const devsDetalle = devs.filter(dv => dv.item_id === it.id).map(dv => {
        // dv.fecha es el día real de la entrega (queda en el registro);
        // si acordaron cobrar solo hasta una fecha anterior, manda esa
        const dias = diasCobrables(inicioItem, dv.cobrar_hasta || dv.fecha, alq.cobra_sabado, alq.cobrar_primer_dia, dv.cobrar_ultimo_dia);
      ...
      if (pendientes > 0) {
        diasPend = diasCobrables(inicioItem, fechaTope, alq.cobra_sabado, alq.cobrar_primer_dia, true);
