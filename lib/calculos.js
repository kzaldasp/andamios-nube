  ...
  for (const alq of alqs) {
    ...
    const detalleItems = items.filter(it => it.alquiler_id === alq.id).map(it => {
      ...
      const devsDetalle = devs.filter(dv => dv.item_id === it.id).map(dv => {
        ...
        const dias = diasCobrables(alq.fecha_inicio, dv.cobrar_hasta || dv.fecha, alq.cobra_sabado, alq.cobrar_primer_dia, dv.cobrar_ultimo_dia);
      ...
      if (pendientes > 0) {
        diasPend = diasCobrables(alq.fecha_inicio, fechaTope, alq.cobra_sabado, alq.cobrar_primer_dia, true);
