...
import { hashPin, esPinHasheado } from './pines.js';
...
const ESQUEMA = [
  ...
  // Índices para que las consultas sigan rápidas cuando crezcan los datos
  ...
  `CREATE INDEX IF NOT EXISTS idx_items_alquiler ON alquiler_items(alquiler_id)`,
  `CREATE INDEX IF NOT EXISTS idx_devoluciones_item ON devoluciones(item_id)`,
  `CREATE INDEX IF NOT EXISTS idx_pagos_alquiler ON pagos(alquiler_id)`,
...
export const CONFIG_DEFECTO = {
  ...
  precio_tablon: '25',
  // Formato global de los documentos; vacío = usar el formato que trae la app
  plantilla_pagare: '',
  plantilla_recibo: ''
};
...
export async function db() {
  ...
  if (!inicializacion) {
    inicializacion = (async () => {
      ...
      // Formato propio de los documentos de un alquiler (vacío = usar el global)
      await c.execute(`ALTER TABLE alquileres ADD COLUMN plantilla_pagare TEXT DEFAULT ''`).catch(() => {});
      await c.execute(`ALTER TABLE alquileres ADD COLUMN plantilla_recibo TEXT DEFAULT ''`).catch(() => {});
      // Piezas agregadas después del inicio: cobran desde su propia fecha
      await c.execute(`ALTER TABLE alquiler_items ADD COLUMN fecha_inicio TEXT DEFAULT NULL`).catch(() => {});
      // Límite de intentos de PIN por dirección IP (antes era global)
      await c.execute(`ALTER TABLE intentos_login ADD COLUMN ip TEXT DEFAULT ''`).catch(() => {});
      // PINs guardados en texto plano por versiones anteriores: se convierten a hash
      const usuarios = await c.execute('SELECT id, pin FROM usuarios');
      for (const u of usuarios.rows) {
        if (!esPinHasheado(u.pin)) {
          await c.execute({ sql: 'UPDATE usuarios SET pin = ? WHERE id = ?', args: [hashPin(u.pin), u.id] });
        }
      }
