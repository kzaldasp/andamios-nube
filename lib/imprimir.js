...
export async function paginaCalendario(alqId) {
  ...
  // Estado de cuenta día por día: ¿al terminar ese día el cliente debía,
  // estaba al día o tenía saldo a favor? saldo del día = cargos acumulados −
  // pagos acumulados a esa fecha. El descuento es una cortesía final, así que
  // recién resta desde el día del cierre (o de hoy si sigue activo).
  {
    let cumCargo = 0, cumPago = 0;
    const [yi, mi, di] = rangoIni.split('-').map(Number);
    const [yf, mf, df] = rangoFin.split('-').map(Number);
    for (let d = new Date(yi, mi - 1, di), fin = new Date(yf, mf - 1, df); d <= fin; d.setDate(d.getDate() + 1)) {
      const iso = isoFecha(d.getFullYear(), d.getMonth() + 1, d.getDate());
      const info = dias.get(iso);
      if (info) { cumCargo += info.monto; cumPago += info.pago; }
      const saldo = cumCargo - cumPago - (iso >= fechaTope ? alq.descuento : 0);
      if (saldo > 0) tocar(iso).debe = true;
      else if (saldo < 0) tocar(iso).favor = true;
    }
  }
  ...
  const calendarios = mesesEntre(rangoIni, rangoFin).map(([y, m]) => {
    ...
    for (let d = 1; d <= ndias; d++) {
      ...
      let clase = 'fuera';
      if (dentro) clase = dow === 0 ? 'nocobra' : (info?.debe ? 'debe' : info?.favor ? 'favor' : 'aldia');
      const evs = [];
...
<table>
  ...
  table.cal td.debe { background: #fdeaea; }
  table.cal td.aldia { background: #eaf6ea; }
  table.cal td.favor { background: #e8f0fd; }
...
<p class="leyenda">
  <span><i class="sw" style="background:#fdeaea"></i> debiendo ese día</span>
  <span><i class="sw" style="background:#eaf6ea"></i> al día / sin deuda</span>
  <span><i class="sw" style="background:#e8f0fd"></i> a favor (pagó de más)</span>
  <span>$ = cobro del día · — = no se cobra (domingo${alq.cobra_sabado ? '' : ' / sábado'})</span>
</p>
<p class="leyenda">
