...
export async function paginaCalendario(alqId) {
  ...
  // Pintado día por día según lo cubierto por los abonos: los pagos (más el
  // descuento) se van sumando y "cubren" los cargos desde el inicio. Cada día
  // queda verde si los cargos acumulados hasta ahí ya están cubiertos, o rojo
  // si todavía falta pagarlo. Más sencillo de leer que el saldo neto diario.
  {
    const cubierto = calc.pagos.reduce((s, p) => s + p.monto, 0) + alq.descuento;
    let cumCargo = 0;
    ...
    for (let d = new Date(yi, mi - 1, di), fin = new Date(yf, mf - 1, df); d <= fin; d.setDate(d.getDate() + 1)) {
      ...
      if (info) cumCargo += info.monto;
      if (cumCargo > cubierto) tocar(iso).debe = true;
    }
  }
  ...
  const calendarios = mesesEntre(rangoIni, rangoFin).map(([y, m]) => {
    ...
    for (let d = 1; d <= ndias; d++) {
      ...
      let clase = 'fuera';
      if (dentro) clase = dow === 0 ? 'nocobra' : (info?.debe ? 'debe' : 'aldia');
      const evs = [];
...
<p class="leyenda">
  <span><i class="sw" style="background:#eaf6ea"></i> pagado (cubierto por los abonos)</span>
  <span><i class="sw" style="background:#fdeaea"></i> pendiente de pago</span>
