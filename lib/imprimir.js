...
export async function paginaCalendario(alqId) {
  ...
  // Estado de deuda día por día: ¿al terminar ese día el cliente debía algo?
  // saldo del día = cargos acumulados − descuento − pagos acumulados a esa fecha.
  {
    ...
    for (let d = new Date(yi, mi - 1, di), fin = new Date(yf, mf - 1, df); d <= fin; d.setDate(d.getDate() + 1)) {
      ...
      if (cumCargo - alq.descuento - cumPago > 0) tocar(iso).debe = true;
    }
  }
  ...
  const calendarios = mesesEntre(rangoIni, rangoFin).map(([y, m]) => {
    ...
    for (let d = 1; d <= ndias; d++) {
      ...
      let clase = 'fuera';
      if (dentro) clase = info?.debe ? 'debe' : 'aldia';
      const evs = [];
